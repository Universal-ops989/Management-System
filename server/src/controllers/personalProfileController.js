...
import { personalProfilePermissions } from '../utils/permissions/entityPermissions.js';
import { sanitizePersonalProfile } from '../utils/sanitizer.js';
 ...
 */
const validatePersonalProfile = (data, isUpdate = false) => {
  const errors = [];
...
export const listPersonalProfiles = async (req, res, next) => {
  try {
    ...
    res.json(createSuccessResponse({
      profiles: accounts.map(a => sanitizePersonalProfile(req.user, a)),
      pagination: {
        page: Number(page),
        limit: Number(limit),
        total,
...
/**
 * Get single personal profile
 * View: OWNER + ADMIN + BOSS
 */
export const getPersonalProfile = async (req, res, next) => {
  try {
    const account = await PersonalProfile.findById(req.params.id)
      .populate('ownerUserId', 'name email')
      .populate('pictureFileId')
      .populate('attachments');
    ...
    if (!personalProfilePermissions.canView(req.user, account)) {
      return res.status(403).json(createErrorResponse('ACCESS_DENIED', 'No permission', 403));
    }

    res.json(createSuccessResponse({
      profile: (req.user, account.toObject())
    }));
  } catch (err) {
    next(err);
  }
 ...
 */
export const createPersonalProfile = async (req, res, next) => {
  try {
    if (!personalProfilePermissions.canCreate(req.user)) {
      return res.status(403).json(createErrorResponse('ACCESS_DENIED', 'No permission', 403));
    }

    const errors = validatePersonalProfile(req.body);
    if (errors.length) {
      return res.status(400).json(createErrorResponse('VALIDATION_ERROR', errors[0], 400));
    }

    const account = new PersonalProfile({
      name: req.body.name,
    ...
    await auditLog(req, 'PERSONAL_PROFILE_CREATE', 'PERSONAL_PROFILE', account._id.toString(), {
      ...getRequestMeta(req),
      name: account.name
    });

    res.status(201).json(createSuccessResponse({
      profile: sanitizePersonalProfile(req.user, account.toObject())
    }));
  } catch (err) {
    next(err);
  }
 ...
 * Edit: OWNER + ADMIN
 */
export const updatePersonalProfile = async (req, res, next) => {
  try {
    const account = await PersonalProfile.findById(req.params.id);

    if (!account) {
      return res.status(404).json(createErrorResponse('NOT_FOUND', 'Personal profile not found', 404));
    }

    const editPermission = personalProfilePermissions.canEdit(req.user, account);
    if (!editPermission.canEdit) {
      return res.status(403).json(createErrorResponse('ACCESS_DENIED', editPermission.reason, 403));
    }

    const errors = validatePersonalProfile(req.body, true);
    if (errors.length) {
      return res.status(400).json(createErrorResponse('VALIDATION_ERROR', errors[0], 400));
    }
    ...
    await auditLog(req, 'PERSONAL_PROFILE_UPDATE', 'PERSONAL_PROFILE', account._id.toString(), {
      ...getRequestMeta(req),
      name: account.name
    });
...
export const uploadPersonalProfilePicture = async (req, res, next) => {
  try {
    const profile = await PersonalProfile.findById(req.params.id);
    ...
    const editPermission = personalProfilePermissions.canEdit(req.user, profile);
    if (!editPermission.canEdit) {
    ...
    const upload = createModuleUpload('personal_profiles');
    ...
    singleUpload(req, res, async (err) => {
      ...
      try {
        ...
        await auditLog(req, 'PERSONAL_PROFILE_PICTURE_UPLOAD', 'PERSONAL_PROFILE', profile._id.toString(), {
          ...getRequestMeta(req),
          fileName: fileMeta.originalName
        });

        const sanitized = sanitizePersonalProfile(req.user, profile.toObject ? profile.toObject() : profile);
 ...
 */
export const deletePersonalProfile = async (req, res, next) => {
  try {
    const account = await PersonalProfile.findById(req.params.id);
    ...
    if (!personalProfilePermissions.canDelete(req.user, account)) {
      return res.status(403).json(createErrorResponse('ACCESS_DENIED', 'No permission', 403));
    }

    await account.deleteOne();

    await auditLog(req, 'PERSONAL_PROFILE_DELETE', 'PERSONAL_PROFILE', account._id.toString(), {
      ...getRequestMeta(req),
      name: account.name
    });
    ...
    singleUpload(req, res, async (err) => {
      ...
      try {
        ...
        await profile.populate('attachments');
          ...
          await auditLog(req, 'PERSONAL_PROFILE_ATTACHMENT_UPLOAD', 'PERSONAL_PROFILE', profile._id.toString(), {
          ...getRequestMeta(req),
          fileName: fileMeta.originalName
        });
  ...
  try {
    ...
    multipleUpload(req, res, async (err) => {
      ...
      try {
        ...
        await auditLog(req, 'PERSONAL_PROFILE_ATTACHMENTS_UPLOAD', 'PERSONAL_PROFILE', profile._id.toString(), {
          ...getRequestMeta(req),
...
export const deleteAttachment = async (req, res, next) => {
  try {
    ...
    if (!personalProfilePermissions.canEdit(req.user, account).canEdit) {
      return res.status(403).json(createErrorResponse('ACCESS_DENIED', 'No permission', 403));
    }
